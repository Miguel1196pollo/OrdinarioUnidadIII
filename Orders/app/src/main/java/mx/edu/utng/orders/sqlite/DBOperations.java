...
public final class DBOperations {
    ...
    public Cursor getPlatform() {
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.PLATFORM);
        return database.rawQuery(sql, null);
    }
    public Cursor getPlatformById(String idPlatform) {
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s WHERE %s=?",
                OrderDB.Tables.PLATFORM,
                OrderContract.Platforms.ID);
        String[] whereArgs ={idPlatform};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertPlatform(Platform platform) {
        SQLiteDatabase database = db.getWritableDatabase();
        String idPlatform = OrderContract.Platforms.generateIdPlatform();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Platforms.ID, idPlatform);
        values.put(OrderContract.Platforms.IMG_FILENAME, platform.getImgFilename());
        values.put(OrderContract.Platforms.NAME, platform.getName());


        return database.insertOrThrow(OrderDB.Tables.PLATFORM,
                null, values) > 0 ? idPlatform : null;
    }
    ...
    public boolean updatePlatform(Platform platform) {
        ...
        ContentValues values = new ContentValues();
        values.put(OrderContract.Platforms.IMG_FILENAME,
                platform.getImgFilename());
        values.put(OrderContract.Platforms.NAME,
                platform.getName());
        String whereClause = String.format("%s=?",
                OrderContract.Platforms.ID);
        final String[] whereArgs = {platform.getIdPlatform()};
        int result = database.update(OrderDB.Tables.PLATFORM,
                values, whereClause, whereArgs);
        return result > 0;
    }
    public boolean deletePlatform(String idPlatform) {
        ...
        String whereClause = String.format("%s=?",
                OrderContract.Platforms.ID);
        final String[] whereArgs = {idPlatform};
        int result = database.delete(OrderDB.Tables.PLATFORM, whereClause, whereArgs);
        return result > 0;
    }
